...
class FordFulkersonAlgorithm:
    ...
    def createGraph(self, graph = None,augPath=None,flag=0):
        self.G = pydot.Dot(graph_name="ford", graph_type="digraph",rankdir="LR",nodesep=0.5, pad=.1, bgcolor="black")
        if graph == None and augPath == None:
            for k in self.graph:
                node = pydot.Node(k,fontname="Bold",style="filled", fillcolor="gray")
                self.G.add_node(node)
            for k, j in  self.graph.items():
                for e in range (1,len(j)):
                    st = str(j[e][2])+'/' + str(j[e][1])
                    edge = pydot.Edge(k, j[e][0],label=st,fontname="Bold",arrowhead='vee', fontcolor="white",color="white")
                    self.G.add_edge(edge)
            self.augPathsGraph.append(self.G)
        elif augPath == None:
            G = pydot.Dot(graph_name="ford", graph_type="digraph",rankdir="LR",nodesep=.5, pad=.1, bgcolor="black")
            for k in graph:
                node = pydot.Node(k,fontname="Bold",style="filled", fillcolor="gray")
                G.add_node(node)
...
#             display(im)
            ...
            for i in range(len(augPath)-1):
                ...
                edge = pydot.Edge(augPath[i][0],augPath[i+1][0],label=temp[i],\
                                  fontname="Bold",arrowhead='vee',color='red', fontcolor='red' )
                G.add_edge(edge)
            self.augPathsGraph.append(G)
